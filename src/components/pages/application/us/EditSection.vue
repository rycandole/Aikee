...
const showInformation = async () => {
  ...
  for (var i = 0; i < showApplication.length; i++) {
    ...
    validate_first_dose.value =
      showApplication[i].first_dose == "Invalid date"
        ? ""
        : showApplication[i].first_dose || "";
    secondDose.value = showApplication[i].second_dose || "";
    validate_second_dose.value =
      showApplication[i].second_dose == "Invalid date"
        ? ""
        : showApplication[i].second_dose || "";
    ...
    validate_add_passport_date.value = showApplication[i].IssueDate || "";
    validate_passport_expiration_date.value = showApplication[i].ValidDate || "";
    ...
    ad_prev_medical_exam_month.value =
      moment(new Date(showApplication[i].PrevMed_Date)).format("MMMM") || "";
    ad_prev_medical_exam_year.value =
      moment(new Date(showApplication[i].PrevMed_Date)).format("YYYY") || "";
    ad_prev_xray_month.value =
      moment(new Date(showApplication[i].Prev_CXRDate)).format("MMMM") || "";
    ad_prev_xray_year.value =
      moment(new Date(showApplication[i].Prev_CXRDate)).format("YYYY") || "";
...
const schema = yup.object().shape({
  ...
  ad_birth_country: yup
    ...
    .max(50, "Birth country must be at most 50 characters"),
  // .matches(nameRegex, "Please avoid using numbers and special characters ex: !@#$%^"),
  ad_mother_name: yup
    .string()
    .required("Last name is required!")
